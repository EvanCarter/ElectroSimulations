...
class PhaseShiftSceneV2(Scene):
    """
    Clean physics simulation showing voltage phase shifts as coils move.
    
    V2 Changes:
    - Reduced duration (fewer rotations)
    - Graphs draw out from origin (accumulate) rather than scrolling window
    """
    ...
    def construct(self):
        ...
        lookup_steps = 50000  # Ultra high resolution lookup table
        flux_lookup = []
        ...
        def get_smoothed_flux_at_angle(t, coil_angle):
            ...
            # The coil is at coil_angle. The magnet is at angle_delta (relative to start).
            # If coil is further CW (smaller angle), magnet must travel further to reach it.
            # So effective time is LESS (we are effectively earlier in the magnet's cycle relative to coil?)
            # No, if coil is at 12-offset, Magnet needs +time to reach it.
            # So we subtract offset from effective magnet progress?
            # Re-derived: t_eff = t - offset/w. offset = PI/2 - coil_angle.
...
class PhaseShiftSceneV3(Scene):
    """
    Same physics as V2 but with 3 separate graphs (one per coil).
    """
    ...
    def construct(self):
        ...
        def moving_1x_profile(t):
            MOVE_START = 2.0
            MOVE_DURATION = 5.0
            TARGET_OFFSET = 120 * DEGREES  # 4 o'clock position
        ...
        def moving_2x_profile(t):
            return 2.0 * moving_1x_profile(t)  # 8 o'clock position (240°)
        ...
        print(f"V3: Calculating physics for {len(coils_config)} coil(s)...")
        ...
        generator_group = VGroup(stator, rotor_group, coil_mobjects, coil_labels)
        generator_group.scale(0.75)  # Scale down to make room for 3 graphs
        generator_group.to_edge(LEFT, buff=0.8)
        disk_center = rotor_group.get_center()

        # --- VOLTAGE GRAPHS (Right side - 3 separate graphs) ---
        graph_height = 1.3
        graph_spacing = 0.3
        ...
        for i, coil_cfg in enumerate(coils_config):
            ...
            label = Text(f"Coil {coil_cfg.name}", font_size=18, color=coil_cfg.color)
            label.next_to(ax, UP, buff=0.08)

            ax_group = VGroup(ax, label)
            axes_dict[coil_cfg.name] = ax
            graph_group.add(ax_group)

        graph_group.arrange(DOWN, buff=graph_spacing)
        graph_group.to_edge(RIGHT, buff=0.5)

        voltage_title = Text("Induced Voltage", font_size=26).next_to(graph_group, UP, buff=0.15)
        graph_group.add(voltage_title)

        # --- VOLTAGE CURVES (one per graph) ---
        curves = VGroup()
        for coil_cfg in coils_config:
            curve = VMobject().set_color(coil_cfg.color).set_stroke(width=2)
            curve.coil_name = coil_cfg.name
        ...
        for coil in coil_mobjects:
            def update_coil_position(mob):
                ...
                new_pos = disk_center + np.array([
                    MAGNET_PATH_RADIUS * 0.75 * math.cos(angle),
                    MAGNET_PATH_RADIUS * 0.75 * math.sin(angle),
                    0
                ])
                mob.move_to(new_pos)
        ...
        # Voltage curve updaters - each curve uses its own axes
        for curve in curves:
            def update_curve(mob):
                t_now = time_tracker.get_value()
                trace = coil_data[mob.coil_name]
                ax = axes_dict[mob.coil_name]
                ...
                points = []
                for t_val, v_val in visible_subsampled:
                    points.append(ax.c2p(t_val, v_val))
