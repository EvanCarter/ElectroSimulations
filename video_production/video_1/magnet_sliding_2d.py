...
class MagnetSliding2D(Scene):
    def construct(self):
        ...
        # Flux (Top)
        flux_axes = Axes(
            x_range=[0, 5, 1], # Extended for slower animation
            y_range=[0, 4, 1], # Approx max flux: B=1.5 * pi*0.6^2 ~= 1.7. Maybe go higher?
                               # Area = pi * 0.36 ~= 1.13. B=1.5 -> 1.7. B=1.0 -> 1.13.
            x_length=5,
            y_length=2.5,
        ...
        # --- Scenario 1: Baseline ---
        # Blue, T=3s (Slower), B=1.0
        run_scenario("Baseline", BLUE, 3.0, 1.0)
        self.wait(0.5)

        # --- Scenario 2: Stronger Magnet ---
        # Purple, T=3s (Slower), B=2.0
        run_scenario("Stronger Magnet", PURPLE, 3.0, 2.0)
        self.wait(0.5)

        # --- Scenario 3: Faster Magnet ---
        # Yellow, T=1.5s (Slower than before), B=1.0
        run_scenario("Faster Movement", YELLOW, 1.5, 1.0)
